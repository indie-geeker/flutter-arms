...
- `lib/src/core/`: app-wide constants and shared primitives.
...
- Feature `presentation` may import its own feature `domain`/`di` and `core` abstractions.
- Feature-to-feature communication must go through `core/shared` abstractions (no internal cross imports).
