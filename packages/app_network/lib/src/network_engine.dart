...
class NetworkEngine implements INetworkEngine {
  ...
  // 请求去重相关
  final Map<String, Future<ResponseWrapper>> _pendingRequests = {};
  ...
  Future<ResponseWrapper<T>> request<T>(
    ...
    if (_pendingRequests.containsKey(requestKey)) {
      ...
      return _pendingRequests[requestKey]! as Future<ResponseWrapper<T>>;
    }
  ...
  /// 执行带拦截器链的请求
  Future<ResponseWrapper<T>> _executeRequestWithInterceptors<T>(
      RequestOptions options,
      Object? cancelToken
      ) async {
    var processedOptions = options;
    final dio.CancelToken? ct = cancelToken as dio.CancelToken?;
    // 最大尝试次数：初次 + 重试次数
    final int maxAttempts = (_config.retryCount) + 1;
    int attempt = 0;
    ...
    while (true) {
      ...
      if (ct?.isCancelled == true) {
        throw NetworkException(
          message: NetworkErrorType.cancel.message,
          code: null,
          statusCode: null,
        );
      }
      ...
      try {
        ...
        for (final interceptor in _getSortedInterceptors()) {
          if (interceptor.enabled) {
            responseWrapper = await interceptor.onResponse<T>(responseWrapper, processedOptions);
          }
        }
        ...
        // 5) 若需重试：拦截器通过 RetryableNetworkException 告知
        if (processedError is RetryableNetworkException) {
          // 达到最大尝试次数则不再重试，抛出原始错误或映射后错误
          if (attempt >= maxAttempts) {
            final original = processedError.originalError;
            if (original is dio.DioException) {
              throw _mapDioException(original);
            }
            throw original;
          }
          ...
          // 使用拦截器提供的新请求参数进行下一轮重试
          processedOptions = processedError.retryOptions;
          continue; // 进入下一轮 while，完成实际重试
        }
  ...
  @override
  Future<ResponseWrapper<T>> get<T>(String path,
  ...
  @override
  Future<ResponseWrapper<T>> post<T>(String path,
      {data,
        Map<String, dynamic>? queryParameters,
        RequestOptions? options,
        Object? cancelToken}) {
  ...
  @override
  Future<ResponseWrapper<T>> put<T>(String path,
      {data,
        Map<String, dynamic>? queryParameters,
        RequestOptions? options,
        Object? cancelToken}) {
  ...
  @override
  Future<ResponseWrapper<T>> patch<T>(String path,
      {data,
        Map<String, dynamic>? queryParameters,
        RequestOptions? options,
        Object? cancelToken}) {
  ...
  @override
  Future<ResponseWrapper<T>> delete<T>(String path,
      {data,
        Map<String, dynamic>? queryParameters,
        RequestOptions? options,
        Object? cancelToken}) {
  ...
  @override
  Future<ResponseWrapper<String>> download(String url, String savePath,
    ...
    switch (e.type) {
      case dio.DioExceptionType.connectionTimeout:
        return NetworkException(
          message: e.message ?? NetworkErrorType.connectionTimeout.message,
          code: null,
          statusCode: e.response?.statusCode,
          details: NetworkErrorType.connectionTimeout,
        );
      case dio.DioExceptionType.receiveTimeout:
        return NetworkException(
          message: e.message ?? NetworkErrorType.receiveTimeout.message,
          code: null,
          statusCode: e.response?.statusCode,
          details: NetworkErrorType.receiveTimeout,
        );
      case dio.DioExceptionType.badResponse:
        return NetworkException(
          message: e.message ?? NetworkErrorType.serverError.message,
          code: null,
          statusCode: e.response?.statusCode,
          details: NetworkErrorType.serverError,
        );
      case dio.DioExceptionType.cancel:
        return NetworkException(
          message: e.message ?? NetworkErrorType.cancel.message,
          code: null,
          statusCode: e.response?.statusCode,
          details: NetworkErrorType.cancel,
        );
      default:
        return NetworkException(
          message: e.message ?? NetworkErrorType.unknown.message,
          code: null,
          statusCode: e.response?.statusCode,
          details: NetworkErrorType.unknown,
        );
    }
  }
