...
void main(List<String> argv) {
  ...
  final name =
      flags['name'] ?? _ask('Project name (snake_case)', _validateName);
  final pkg =
      flags['package'] ??
      _ask('Android package (e.g. com.example.my_app)', _validatePackage);
  ...
  stdout
    ...
    ..writeln(
      '  4) Replace assets/icon/app_icon.png and assets/splash/logo.png, then:',
    )
...
void _renameAndroid(String pkg, String display) {
  _editFile(
    _path('android/app/build.gradle.kts'),
    (s) => s
        .replaceAll(_oldPackage, pkg)
        .replaceAll(
          'resValue("string", "app_name", "$_oldDisplay Dev")',
          'resValue("string", "app_name", "$display Dev")',
        )
        .replaceAll(
          'resValue("string", "app_name", "$_oldDisplay")',
          'resValue("string", "app_name", "$display")',
        ),
  );
  _editFile(
    _path('android/app/src/main/AndroidManifest.xml'),
    (s) => s.replaceFirst(
  ...
  final oldDir = Directory(
    '${kotlinRoot.path}/${_oldPackage.replaceAll('.', Platform.pathSeparator)}',
  );
  final newDir = Directory(
    '${kotlinRoot.path}/${pkg.replaceAll('.', Platform.pathSeparator)}',
  );
  ...
  if (oldDir.existsSync() && oldDir.path != newDir.path) {
    ...
    for (final entity in oldDir.listSync()) {
      final target =
          '${newDir.path}${Platform.pathSeparator}${_basename(entity.path)}';
  ...
  final mainActivity = File(
    '${newDir.path}${Platform.pathSeparator}MainActivity.kt',
  );
...
void _renameIos(String iosId, String display, String snakeName) {
  _editFile(
    _path('ios/Runner.xcodeproj/project.pbxproj'),
...
void _renameMacos(String iosId, String display, String snakeName, String pkg) {
  _editFile(
    _path('macos/Runner/Configs/AppInfo.xcconfig'),
    (s) => s
  ...
  _editFile(
    _path('macos/Runner.xcodeproj/project.pbxproj'),
    (s) => s.replaceAll(_oldIosId, iosId),
  );
...
void _renameWindows(String snakeName, String display, String pkg) {
  ...
  _editFile(
    _path('windows/runner/Runner.rc'),
    (s) => s
        .replaceFirst(
...
void _validatePackage(String p) {
  if (!RegExp(r'^[a-z][a-zA-Z0-9_]*(\.[a-z][a-zA-Z0-9_]*)+$').hasMatch(p)) {
    throw _RenameException(
      'Package must look like com.example.my_app (got "$p")',
    );
  }
}
...
String _toCamelPackage(String pkg) => pkg
    .split('.')
    .map((seg) {
...
Directory _projectRoot() {
  ...
  while (!File(
    ...
    if (dir.parent.path == dir.path) {
      stderr.writeln(
        '[rename] Could not locate project root (no pubspec.yaml found)',
      );
      exit(1);
    }
    dir = dir.parent;
...
void _guardAlreadyRenamed(String newName) {
  ...
  if (current == newName) {
    stdout.writeln(
      '[rename] pubspec already shows name=$newName; running in idempotent mode.',
    );
    return;
  }
  if (current != _oldName) {
